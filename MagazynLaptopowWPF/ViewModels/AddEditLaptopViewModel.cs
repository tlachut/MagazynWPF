...
namespace MagazynLaptopowWPF.ViewModels
{
    public class AddEditLaptopViewModel : BaseViewModel, IDataErrorInfo
    {
        ...
        {
            get => _laptop.KodKreskowy ?? string.Empty;
            set
            {
                if (_laptop.KodKreskowy != value)
                {
                    _laptop.KodKreskowy = value;
                    OnPropertyChanged();
                    ValidateProperty(nameof(KodKreskowy));
                }
            }
        }
        ...
        private void ValidateProperty(string propertyName)
        {
            ...
            switch (propertyName)
            {
                case nameof(KodKreskowy):
                    if (KodKreskowy?.Length > 50)
                    {
                        _errors[propertyName] = "Kod kreskowy nie może mieć więcej niż 50 znaków.";
                    }
                    break;
